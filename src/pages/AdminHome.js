function AdminHome({ setPage, products, orders, users, goBack }) {
  ...
  const stats = [
    { label: '전체 상품', value: products.length + '개', color: '#00a85e', bg: '#e8faf3',
    ...
    { label: '전체 주문', value: orders.length + '건', color: '#1a73e8', bg: '#e8f0fe',
    ...
    { label: '전체 회원', value: users.length + '명', color: '#e65100', bg: '#fff3e0',
    ...
    { label: '오늘 매출', value: '₩' + todaySales.toLocaleString(), color: '#c62828', bg: '#fce4ec',
  ...
  return (
    <div style={{ background: '#f8fffe', minHeight: '100vh', paddingBottom: '80px' }}>
      ...
      <div style={{ margin: '-24px 16px 0', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', position: 'relative', zIndex: 2 }}>
        {stats.map((stat) => (
          <div key={stat.label} style={{ background: 'white', borderRadius: '18px', padding: '16px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', gap: '12px' }}>
      ...
      <div style={{ margin: '16px 16px 0' }}>
        <p style={{ fontSize: '15px', fontWeight: '800', color: '#1a1a1a', margin: '0 0 12px' }}>빠른 메뉴</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
          {menus.map((menu) => (
            <div key={menu.label} onClick={() => setPage(menu.page)}
              style={{ background: 'white', borderRadius: '16px', padding: '16px 10px', cursor: 'pointer', textAlign: 'center', boxShadow: '0 2px 12px rgba(0,0,0,0.05)', border: '1px solid #f0faf5', transition: 'all 0.2s' }}>
              <div style={{ width: '44px', height: '44px', background: '#f0faf5', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 8px' }}>
                {menu.icon}
              </div>
              <p style={{ fontSize: '12px', fontWeight: '700', color: '#1a1a1a', margin: '0 0 3px', wordBreak: 'keep-all' }}>{menu.label}</p>
      ...
      <div style={{ margin: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <p style={{ fontSize: '15px', fontWeight: '800', color: '#1a1a1a', margin: 0 }}>최근 주문 내역</p>
        ...
        {orders.length === 0 ? (
          <div style={{ background: 'white', borderRadius: '16px', padding: '40px 20px', textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
            <div style={{ width: '56px', height: '56px', background: '#f0faf5', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
            ...
            {orders.slice(0, 5).map((order) => {
              ...
              return (
                <div key={order.id} onClick={() => setPage('adminOrders')}
                  style={{ background: 'white', borderRadius: '16px', padding: '14px 16px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', cursor: 'pointer', border: '1px solid #f0faf5' }}>
