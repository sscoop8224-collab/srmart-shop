              ...
              {filtered.map(product => {
                ...
                return (
                  <div key={product.id} onClick={() => onProductClick?.(product)} style={{ background: cardBg, borderRadius: 16, overflow: 'hidden', cursor: 'pointer', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: `1px solid ${border}` }}>
                    <div style={{ width: '100%', aspectRatio: '1', overflow: 'hidden', background: '#f6f7f8' }}>
                      <img src={product.image ? imgUrl(product.image) : getCategoryImage(product.large)} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block' }} onError={e => { e.currentTarget.src = getCategoryImage(product.large); }} />
                    </div>
